...
$query = "SELECT fullname, exam_score, exam_score2 FROM tbl_users where year=1 and role_id = 2";
$result = mysqli_query($conn, $query);
$data = mysqli_fetch_all($result, MYSQLI_ASSOC);

$below75Score = array_filter($data, function ($row) {
    return $row['exam_score'] < 75;
});

$below75Score2 = array_filter($data, function ($row) {
    return $row['exam_score2'] < 75;
});
?>
...
<body>
    <h2 style="margin-top: 30px; margin-left: 135px;padding: 15px; height: 50px;  font-size: 18px; color: #34425A; border-radius: 2px; border-color: white; border-width: 1px;  position: absolute; background-color: #f6f6f6; border-style: outset">CCE 101</h2>
        ...
        var barChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: <?php echo json_encode(array_column($below75Score, 'exam_score')); ?>,
                    backgroundColor: 'rgba(75, 192, 192, 0.2)',
                    borderColor: 'rgba(75, 192, 192, 1)',
                    borderWidth: 1
                }, {
                    label: 'Second Exam Score ',
                    data: <?php echo json_encode(array_column($below75Score, 'exam_score2')); ?>,
