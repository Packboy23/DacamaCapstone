...
<div style="margin-top: 120px; margin-left: 210px"><br>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "SELECT fullname, year, elec, elec2 FROM tbl_users WHERE  (elec AND elec2 < $selectedScore) or (elec IS NOT NULL AND elec2 ) AND year = 2 and role_id = 2 ";
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        <td>{$row['fullname']}</td>
                        <td style='width: 120px'><center>{$row['elec']}</center></td>
                        <td style='width: 120px'><center>{$row['elec2']}</center></td>
                      </tr>";
            }
