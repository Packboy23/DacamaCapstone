...
<div style="margin-top: 120px; margin-left: 210px"><br>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "SELECT fullname, year, netw, netw2 FROM tbl_users WHERE  (netw AND netw2 < $selectedScore) or (netw IS NOT NULL AND netw2 ) AND year = 2 and role_id = 2 ";
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        <td>{$row['fullname']}</td>
                        <td style='width: 120px'><center>{$row['netw']}</center></td>
                        <td style='width: 120px'><center>{$row['netw2']}</center></td>
                      </tr>";
            }
