...
<div style="margin-top: 130px; margin-left: 210px"><br>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "SELECT fullname, year, integ, integ2 FROM tbl_users WHERE  (integ AND integ2 < $selectedScore) or (integ IS NOT NULL AND integ2 ) AND year = 3 and role_id = 2 ";
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        <td>{$row['fullname']}</td>
                        <td style='width: 120px'><center>{$row['integ']}</center></td>
                        <td style='width: 120px'><center>{$row['integ2']}</center></td>
                      </tr>";
            }
